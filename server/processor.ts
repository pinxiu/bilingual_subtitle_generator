...
export const processJobInitial = async (job: Job, updateJob: (id: string, partial: Partial<Job>) => void) => {
  ...
  try {
    // --- NEW SWITCHER LOGIC ---
    // Select script based on job preference
    const scriptName = job.translator === 'gemini' ? 'gemini_service.py' : 'ai_service.py';
    const pythonScript = path.join(process.cwd(), scriptName);
    
    updateJob(job.id, { message: `Starting ${job.translator} engine...` });
    ...
    await new Promise<void>((resolve, reject) => {
      // Arguments match the structure we built for both Python files
      const args = [
        pythonScript, 
        inputPath, 
        srtPath,
      ...
      python.on('error', (err) => {
        reject(new Error(`Failed to spawn Python: ${err.message}. Script: ${scriptName}`));
      });
      ...
      python.stdout.on('data', (data) => {
        ...
        for (const line of lines) {
          ...
          try {
            ...
            console.log(`[Python ${job.translator} Log]: ${line}`);
          }
        }
      });
      ...
      python.stderr.on('data', (data) => {
        ...
        console.error(`[Python ${job.translator} Error]: ${data}`);
      });

      python.on('close', (code) => {
        if (code !== 0) {
          reject(new Error(`AI Service (${scriptName}) failed: ${errorOutput.split('\n').pop() || 'Unknown error'}`));
        } else {
          resolve();
        }
      });
    ...
    if (!fs.existsSync(srtPath)) {
      throw new Error(`SRT file was not generated by ${scriptName}`);
    }

    const srtContent = fs.readFileSync(srtPath, 'utf-8');
    const cues = parseSrt(srtContent);

    // Filter cues based on user output preference (EN, ZH, or Both)
