...
function App() {
  ...
  const [lineCount, setLineCount] = useState<1 | 2>(2); // default bilingual = 2 lines
  ...
  // Keep lineCount meaningful:
  // - bilingual supports 1 or 2
  // - en/zh: lineCount doesn't matter much, but we keep it at 1 to avoid confusion
  useEffect(() => {
    if (outputFormat === 'bilingual') {
      // keep as-is (1 or 2), but default to 2 if somehow invalid
      if (lineCount !== 1 && lineCount !== 2) setLineCount(2);
    } else {
      if (lineCount !== 1) setLineCount(1);
    }
  ...
  const handleStartGeneration = async () => {
    ...
    formData.append('lineCount', lineCount.toString()); // IMPORTANT: always send
  ...
  // Upload existing files (video + srt) -> then apply formatting preferences via /job/:id/load
  const handleResumeSubmit = async () => {
    ...
    try {
      ...
      // Apply output options to the loaded job so SRT/preview matches immediately
      await axios.post(`${API_BASE}/job/${id}/load`, {
        outputFormat,
        lineCount,
        enTranscript,
        zhTranscript,
  ...
  const handleRetranslateSubmit = async () => {
    ...
    formData.append('sourceLang', sourceLang);
    formData.append('outputFormat', outputFormat);
    formData.append('lineCount', lineCount.toString()); // IMPORTANT
    formData.append('enTranscript', enTranscript);
    formData.append('zhTranscript', zhTranscript);
  ...
  // Resume from server job directory: send selection to /job/:id/load
  ...
  const handleEditorContinue = async (config: RenderConfig) => {
    ...
    try {
      ...
      setJobStatus((prev) =>
        prev ? { ...prev, status: 'processing', stage: 'render_soft', message: 'Starting render...' } : null
      );
      resumePolling();
    } catch (err) {
    ...
    <div className="space-y-6">
        ...
        <div className="flex p-1 bg-slate-100 rounded-lg">
          <button
            ...
            className={`flex-1 py-2 text-xs sm:text-sm font-medium rounded-md transition-all ${
              outputFormat === 'en' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'
            }`}
          >
            EN Only
          </button>
          <button
            ...
            className={`flex-1 py-2 text-xs sm:text-sm font-medium rounded-md transition-all ${
              outputFormat === 'zh' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'
            }`}
          >
            ZH Only
          </button>
          <button
            ...
            className={`flex-1 py-2 text-xs sm:text-sm font-medium rounded-md transition-all ${
              outputFormat === 'bilingual' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'
            }`}
          >
            Bilingual
          </button>
      ...
      {outputFormat === 'bilingual' && (
        <div className="space-y-2 animate-in fade-in slide-in-from-top-2 duration-300">
          <label className="text-sm font-bold text-slate-500 uppercase flex items-center gap-2">
            <Type className="w-4 h-4" /> Bilingual Layout
          </label>
          <div className="flex p-1 bg-slate-100 rounded-lg max-w-sm">
            <button
              onClick={() => setLineCount(1)}
              className={`flex-1 py-2 text-sm font-medium rounded-md transition-all ${
                lineCount === 1 ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'
              }`}
            >
              Single Line
            </button>
            <button
              ...
              className={`flex-1 py-2 text-sm font-medium rounded-md transition-all ${
                lineCount === 2 ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'
              }`}
            >
              Two Lines
            </button>
          ...
          <p className="text-xs text-slate-400">
            {lineCount === 1
              ? 'Show “EN / ZH” in one subtitle line.'
              : 'Show English on the first line and Chinese on the second line.'}
          </p>
        </div>
      )}
    </div>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            ...
            <div className="text-xs text-slate-500">Provide text to help AI match and segment more accurately</div>
          </div>
        </div>
        {showTranscripts ? (
    ...
    <div className="min-h-screen bg-slate-50 py-12 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-4xl font-extrabold text-slate-900 tracking-tight mb-2">Bilingual Subtitle Generator</h1>
            ...
            <div className="bg-white p-1 rounded-lg border border-slate-200 shadow-sm flex space-x-1">
              <button
                ...
                className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${
                  activeTab === 'new' ? 'bg-blue-100 text-blue-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                }`}
              >
                Generate New
              </button>
              <button
                ...
                className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${
                  activeTab === 'resume' ? 'bg-blue-100 text-blue-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                }`}
              >
                Resume Project
              </button>
        ...
        <div
          className={`transition-all duration-300 ${
            jobStatus?.status === 'waiting_for_approval'
              ? ''
              : 'bg-white rounded-2xl shadow-xl border border-slate-100 p-6 md:p-8'
          }`}
        >
          {error && (
            <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-lg flex items-start space-x-3">
              ...
              <button
                ...
                className="ml-auto text-red-500 font-bold hover:text-red-700"
              >
                &times;
              </button>
            </div>
              ...
              <div>
                ...
                {file && (
                  <div className="mt-3 flex items-center gap-2 text-sm text-blue-600 bg-blue-50 p-2 rounded-lg border border-blue-100">
                    ...
                    <button onClick={() => setFile(null)} className="ml-auto hover:text-blue-800">
                      Change
                    </button>
                  </div>
                )}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex p-1 bg-slate-100 rounded-lg">
                      <button
                        ...
                        className={`flex-1 py-2 text-sm font-medium rounded-md transition-all ${
                          sourceLang === 'en' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                        }`}
                      >
                        English
                      </button>
                      <button
                        ...
                        className={`flex-1 py-2 text-sm font-medium rounded-md transition-all ${
                          sourceLang === 'zh' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                        }`}
                      >
                        Chinese
                      </button>
          ...
          {!jobId && !jobStatus && activeTab === 'resume' && (
            <div className="space-y-8 animate-in fade-in duration-500">
              {/* Resume formatting controls (IMPORTANT) */}
              <div className="space-y-4">
                <h3 className="font-semibold text-slate-800">Resume Options</h3>
                <SubtitleFormatControls />
                <TranscriptAssistance />
              </div>
                  ...
                  <div className="border-2 border-dashed border-slate-300 rounded-xl p-6 text-center hover:bg-slate-50 transition-colors relative">
                    ...
                    <div className="flex flex-col items-center">
                      ...
                      <span className="font-medium text-slate-700">
                        {resumeVideo ? resumeVideo.name : 'Select Video File'}
                      </span>
                    </div>
                  </div>
                  <div className="border-2 border-dashed border-slate-300 rounded-xl p-6 text-center hover:bg-slate-50 transition-colors relative">
          ...
          {!jobId && !jobStatus && activeTab === 'retranslate' && (
            <div className="space-y-8 animate-in fade-in duration-500">
              <div className="p-4 bg-blue-50 border border-blue-100 rounded-xl text-blue-700 text-sm flex items-start gap-3">
                ...
                <div>
                  <span className="font-bold">Translate-Only Mode:</span> This will take your existing SRT file and use AI
                  to regenerate/improve the translated line(s) while preserving timestamps.
                </div>
              </div>
        ...
        <p className="text-center text-slate-400 text-sm mt-8">Powered by Faster-Whisper, NMT & FFmpeg</p>
      </div>
    </div>
  );
}
